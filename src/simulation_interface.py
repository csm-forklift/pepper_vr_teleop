#!/usr/bin/env python
'''
This node receives the joint command messages and converts them into messages read by the Gazebo simulation. This is designed to allow the controller nodes to publish a single command which can be uesd for both the real Pepper and the simulation by running the corresponding interface node.
'''
...
class SimulationInterface():
    ...
    def jointAnglesCallback(self, msg):
        '''
        Passes the joint angle command to the simulation using the appropriate
        controller topics.
        '''
        #--- Extract the message values to the appropriate angle setpoint
